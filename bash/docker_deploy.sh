...
#       部署前会将临时目录文件备份到 ${dockerdb_dir}/bash/origin_conf 下。
#       部署时切换到 ${dockerdb_dir}/bash/origin_conf 目录执行，并使用当前目录下的 docker-compose.yml。
#       下载镜像时启用并行（最多10个并行任务）。
# ============================================================
...
# 备份 docker-compose.yml 到持久化目录下的 bash/origin_conf 子目录（静默执行）
backup_tmp_to_persist() {
    local target_base="${dockerdb_dir:-/mnt/database}"
    local target_dir="${target_base}/bash/origin_conf"
...
quick_deploy() {
    ...
    # ==================== 修改点1：快捷部署 ====================
    # 切换到持久化目录下的 bash/origin_conf，清除所有现有容器，然后部署
    local deploy_dir="${dockerdb_dir}/bash/origin_conf"
    ...
    echo "正在清除所有现有容器（强制删除）..."
    containers=$(docker ps -aq 2>/dev/null)
    if [ -n "$containers" ]; then
        docker rm -f $containers
        echo "已清除所有容器。"
    else
        echo "没有运行中的容器，跳过清除步骤。"
    fi

    echo "执行: docker compose up -d"
    echo "提示：按 Ctrl+C 可中断部署"
...
menu_pull_images() {
    ...
    local compose_file="${dockerdb_dir}/bash/origin_conf/docker-compose.yml"
    if [ ! -f "$compose_file" ]; then
        echo "错误: 找不到已生成的 docker-compose.yml 文件，请先执行「1.快捷部署」或「2.选择要部署的服务」。"
        return 1
    fi
    ...
    echo "正在拉取所有镜像（并行拉取）... (按 Ctrl+C 中断)"
    if $compose_cmd -f "$compose_file" pull --parallel; then
        echo "所有镜像拉取成功。"
    else
        local exit_code=$?
        if [ $exit_code -eq 130 ] || [ $exit_code -eq 143 ]; then
