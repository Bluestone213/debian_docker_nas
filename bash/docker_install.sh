...
（若源文件非空），然后设置权限 644、所有者 root:root。
然后检测 Docker 是否已安装：
...
# 1. 安装 jq（如果未安装）用于 JSON 处理（后续可能用于读取配置文件，但本脚本中未使用，保留以备扩展）
if ! command -v jq &> /dev/null; then
    echo ">>> 安装 jq (JSON 处理工具)..."
    apt update && apt install -y jq
fi

# 2. 生成最终的 daemon.json（仅复制源文件内容，不添加任何额外字段）
mkdir -p /etc/docker
...
if [ -s "$SOURCE_DAEMON" ]; then
    cp "$SOURCE_DAEMON" /etc/docker/daemon.json
    chmod 644 /etc/docker/daemon.json
    chown root:root /etc/docker/daemon.json
    echo "已从 $SOURCE_DAEMON 复制生成 /etc/docker/daemon.json"
    echo "当前 daemon.json 内容："
    cat /etc/docker/daemon.json
else
    echo "警告: 源文件 $SOURCE_DAEMON 为空，未生成 /etc/docker/daemon.json（Docker 将使用默认配置）"
fi
