    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^\"([a-zA-Z0-9_]+)\"[[:space:]]*:[[:space:]]*\"([^\"]*)\"[,]?[[:space:]]*$ ]]; then
            ...
            case "$key" in
                ...
                jellyfin_mapping_1)     JELLYFIN_MAPPING_1="$value" ;;
                jellyfin_mapping_2)     JELLYFIN_MAPPING_2="$value" ;;
                jellyfin_mapping_3)     JELLYFIN_MAPPING_3="$value" ;;
                ...
                qbittorrent_mapping)    QBITTORRENT_MAPPING="$value" ;;
...
interactive_fill_missing() {
    ...
    if [[ -z "$JELLYFIN_MAPPING_1" ]]; then
        read_from_tty "请输入 jellyfin 映射1（格式 宿主机路径:容器路径）: " JELLYFIN_MAPPING_1
    fi
    if [[ -z "$JELLYFIN_MAPPING_2" ]]; then
        read_from_tty "请输入 jellyfin 映射2（直接回车跳过）: " JELLYFIN_MAPPING_2
    fi
    if [[ -z "$JELLYFIN_MAPPING_3" ]]; then
        read_from_tty "请输入 jellyfin 映射3（直接回车跳过）: " JELLYFIN_MAPPING_3
    fi
    if [[ -z "$SYNCTHING_MAPPING_1" ]]; then
        read_from_tty "请输入 syncthing 映射1（格式 宿主机路径:容器路径）: " SYNCTHING_MAPPING_1
    fi
    ...
    if [[ -z "$QBITTORRENT_MAPPING" ]]; then
        read_from_tty "请输入 qbittorrent 卷映射（格式 宿主机路径:容器路径）: " QBITTORRENT_MAPPING
    fi
    if [[ -z "$ARIA2_DIR" ]]; then
...
while IFS= read -r line <&3; do
	...
	#        # 1. filebrowser
        if [[ "$IMAGE_NAME" == *"filebrowser/filebrowser"* ]]; then
            ...
            # 确保数据目录存在
            if [[ -n "$FILEBROWSER_DIR" ]]; then
                mkdir -p "$FILEBROWSER_DIR" 2>/dev/null || true
            fi

            # 处理 filebrowser 的 config 目录（settings.json）
            fb_config_dir="${dockerdb_dir:-/mnt/database}/filebrowser"
            mkdir -p "$fb_config_dir" 2>/dev/null || true
            settings_file="$fb_config_dir/settings.json"
            if [[ -f "$settings_file" ]]; then
                cp "$settings_file" "${settings_file}.bak"
                echo "已备份 $settings_file 到 ${settings_file}.bak" >&2
            fi
...
EOF
            echo "已生成 $settings_file" >&2

            # 确保端口映射存在（如果缺失则添加）
            port_found=0
            for ((i=LINE_NUM+1; i<=LINE_NUM+20; i++)); do
                port_line=$(sed -n "${i}p" "$COMPOSE_FILE")
                if [[ "$port_line" =~ ^[[:space:]]+ports:[[:space:]]*$ ]]; then
                    port_found=1
                    # 检查是否已有 23333:80 映射
                    if ! sed -n "${i},$((i+10))p" "$COMPOSE_FILE" | grep -q "23333:80"; then
                        # 在 ports: 行后添加映射
                        add_line_after "$i" "- \"23333:80\""
                    fi
                    break
                fi
            done
            if [[ $port_found -eq 0 ]]; then
                echo "警告：未找到 ports 定义，请手动添加端口映射 23333:80" >&2
            fi
        fi
	# 2. jellyfin（改为三个 mapping，与 syncthing 一致）
        if [[ "$IMAGE_NAME" == *"jellyfin/jellyfin"* ]]; then
            HAS_JELLYFIN=1
            for ((i=LINE_NUM+1; i<=LINE_NUM+10; i++)); do
                vol_line=$(sed -n "${i}p" "$COMPOSE_FILE")
                if [[ "$vol_line" =~ ^[[:space:]]+volumes:[[:space:]]*$ ]]; then
                    for map in "$JELLYFIN_MAPPING_1" "$JELLYFIN_MAPPING_2" "$JELLYFIN_MAPPING_3"; do
                        if [[ -n "$map" ]]; then
...
if [[ $HAS_JELLYFIN -eq 1 ]]; then
    echo "  jellyfin 映射1: ${JELLYFIN_MAPPING_1:-无}"
    echo "  jellyfin 映射2: ${JELLYFIN_MAPPING_2:-无}"
    echo "  jellyfin 映射3: ${JELLYFIN_MAPPING_3:-无}"
fi
