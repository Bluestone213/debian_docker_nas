...
parse_compose_config() {
    ...
    # 读取整个文件到数组，便于行号操作和上下文查找
    local lines=()
    mapfile -t lines < "$compose_file"
    local total=${#lines[@]}

    # 第一步：找出所有 container_name: 行及其位置
    declare -A container_lines   # container_name -> 行号（0-index）
    ...
    for cname in "${!container_lines[@]}"; do
        ...
        # 2. 向下查找，直到下一个 container_name: 或文件末尾
        ...
        if [[ "$is_host_mode" == "true" ]]; then
            ...
            CONTAINER_PORTS_MAP["$cname"]=""   # host 模式无端口映射
            continue
        fi
        ...
        for ((k=line_num+1; k<end_line; k++)); do
            ...
            if $in_ports_block; then
                # 检测块结束：遇到非端口映射行且缩进小于等于 ports 的缩进（通常是遇到其他配置项）
                if [[ "$current_line" =~ ^[[:space:]]+[a-zA-Z_-]+: ]]; then
                    in_ports_block=false
                    continue
                fi
                # 提取端口映射：格式 "- " 后跟 "宿主端口:容器端口" 可选 /tcp 或 /udp
...
_view_running_ports() {
    ...
    while IFS= read -r line; do
        if [[ "$line" =~ Docker:[[:space:]]+([a-zA-Z0-9_-]+) ]]; then
            local cname="${BASH_REMATCH[1]}"
            if [[ "$line" =~ port[[:space:]]+([0-9]+)/(tcp|udp) ]]; then
                local port_proto="${BASH_REMATCH[1]}/${BASH_REMATCH[2]}"
                CONTAINER_UFW_PORTS["$cname"]="${CONTAINER_UFW_PORTS[$cname]} $port_proto"
            elif [[ "$line" =~ port[[:space:]]+([0-9]+) ]]; then
                local port="${BASH_REMATCH[1]}"
                CONTAINER_UFW_PORTS["$cname"]="${CONTAINER_UFW_PORTS[$cname]} $port/tcp"
            fi
        fi
    done < <(ufw status 2>/dev/null | grep -i "Docker:")

    echo -e "${YELLOW}容器名称\t\t开放端口（宿主机端口/协议）${NC}"
    echo "----------------------------------------"
