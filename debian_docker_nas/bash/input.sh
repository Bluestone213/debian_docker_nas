...
get_fallback_comment() {
    ...
    case "$key" in
        ...
        jellyfin_dir) echo "Jellyfin 媒体库目录" ;;
        syncthing_mapping_1) echo "Syncthing 映射1 (格式: 宿主机目录:容器目录)" ;;
        syncthing_mapping_2) echo "Syncthing 映射2" ;;
        syncthing_mapping_3) echo "Syncthing 映射3" ;;
        qbittorrent_dir) echo "qBittorrent 下载目录" ;;
...
get_default_value() {
    ...
    # 提取第一个双引号之间的完整内容（包括逗号等）
    if [[ "$line" =~ :[[:space:]]*\"([^\"]*)\" ]]; then
        echo "${BASH_REMATCH[1]}"
    else
        # 如果没有双引号（例如纯数字值），则提取冒号后的非空格部分，去掉可能的尾部逗号
...
part5_keys=(
    "filebrowser_dir" "jellyfin_dir" "syncthing_mapping_1"
    "syncthing_mapping_2" "syncthing_mapping_3" "qbittorrent_dir"
...
# 定义需要导出的合法变量名列表（仅包含字母数字下划线，不含空格或连字符）
# 注意：不使用 readonly，避免多次 source 时报错
EXPORT_KEYS=(
    "Nas_Hostname" "special_config" "type" "dockerdb_dir"
    "Nas_Admin" "Nas_Admin_passwd" "Nas_Root_passwd"
    ...
    "filebrowser_dir" "jellyfin_dir"
    "syncthing_mapping_1" "syncthing_mapping_2" "syncthing_mapping_3"
    "qbittorrent_dir" "aria2_dir" "aria2_passwd"
)
