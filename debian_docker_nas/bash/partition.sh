...
interactive_partition() {
    local disk="$1"
    local total_mib=$(parted "$disk" unit MiB print 2>/dev/null | grep "^Disk $disk" | awk '{print $3}' | tr -d 'MiB')
    if [ -z "$total_mib" ]; then
    ...
    for part in "${parts[@]}"; do
        ...
        local end_mib
        if [ $part_num -eq $part_count ]; then
            end_mib="100%"
        else
            end_mib=$((start_mib + size_mib - 1))
        fi

        parted -s "$disk" mkpart primary ${start_mib}MiB ${end_mib}
        udevadm settle
        ...
        log_info "创建分区 $part_dev (起始 ${start_mib}MiB, 结束 ${end_mib})"
        mkfs.ext4 -F "$part_dev" >/dev/null
        log_info "格式化 $part_dev 为 ext4"
...
mount_existing_partitions() {
    ...
    # 收集所有非系统盘上的 ext4 分区（未挂载的）
    local parts=()
    local part_info=()
    local other_disks=$(lsblk -d -n -o NAME,TYPE | awk '$2=="disk" {print "/dev/"$1}')
    for disk in $other_disks; do
        [ "$disk" = "$sys_disk" ] && continue
        while read -r name fstype mountpoint size label; do
            [ "$fstype" != "ext4" ] && continue
            [ -n "$mountpoint" ] && continue   # 已挂载的跳过
            local part_dev="/dev/$name"
            parts+=("$part_dev")
            part_info+=("$part_dev|${size}|${label:-无标签}")
        done < <(lsblk -ln -o NAME,FSTYPE,MOUNTPOINT,SIZE,LABEL "$disk" 2>/dev/null | grep -v "^$")
    done
    ...
    while true; do
        ...
        local default_mnt=""
        if [ "$label" != "无标签" ] && [ -n "$label" ]; then
            default_mnt="/mnt/${label}"
        else
            local basename=$(basename "$part_dev")
            default_mnt="/mnt/${basename}"
        fi
